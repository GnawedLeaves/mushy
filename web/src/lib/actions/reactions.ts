...
export async function setSaveReaction(saveId: string, reaction: ReactionType | null) {
  ...
  if (reaction === null) {
    ...
    // Runs as this (the actor's) own RLS-scoped client, which is exactly
    // why the notifications insert policy requires actor_id = auth.uid():
    // this insert can only ever be attributed to whoever is actually
    // calling it. Best-effort: a failure here (e.g. self-reaction, which
    // the recipient_id = actor_id case still inserts harmlessly) never
    // blocks the reaction itself from landing.
    const { data: save } = await supabase.from("saves").select("owner_id").eq("id", saveId).maybeSingle();
    if (save && save.owner_id !== user.id) {
      try {
        await supabase.from("notifications").insert({
          recipient_id: save.owner_id,
          actor_id: user.id,
          type: reaction === "like" ? "save_like" : "save_dislike",
          save_id: saveId,
        });
      } catch {
