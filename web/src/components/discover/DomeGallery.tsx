...
// Adapted from reactbits.dev's Dome Gallery (https://reactbits.dev/components/dome-gallery),
// typed for this project and extended with an optional `id` per image so a
// tile can link to its save's detail page. reactbits ships this as
// copy-paste source, not an npm package, so it lives here rather than in
// node_modules -- same treatment as TextPressure on the login/signup pages.
...
interface BuiltItem extends ItemCoord {
  src: string;
  alt: string;
  id: string | null;
  isVideo: boolean;
...
const getDataNumber = (el: HTMLElement, name: string, fallback: number) => {
  const attr = (el.dataset as Record<string, string | undefined>)[name];
  const n = attr == null ? NaN : parseFloat(attr);
  return Number.isFinite(n) ? n : fallback;
};

function buildItems(pool: (string | DomeGalleryImage)[], seg: number): BuiltItem[] {
...
export default function DomeGallery({
  ...
  const onOpenSaveRef = useRef(onOpenSave);
  useEffect(() => {
    onOpenSaveRef.current = onOpenSave;
  }, [onOpenSave]);
  ...
  const openItemFromElement = useCallback(
    (el: HTMLElement) => {
      ...
      // needs a way to actually get from "enlarged image" to the save's own
      // page (comments, reactions, source link).
      const saveId = parent.dataset.id;
      if (saveId) {
        const link = document.createElement("button");
        link.type = "button";
        link.textContent = "View save →";
        link.className = "dome-view-save";
        link.addEventListener("click", (ev) => {
          ev.stopPropagation();
          onOpenSaveRef.current?.(saveId);
        });
